...
import {
  tasksInProject, milestonesInProject, taskById, childrenOf,
  subtaskProgress, isBlocked, isOverdue, milestoneProgress,
  buildProjects, projectLabel, UNSORTED_ID,
} from '../lib/selectors';
...
export default function ProjectDetailView({
  projectId, tasks, milestones = [], today,
  ...
  // project empties out entirely it stops appearing in buildProjects, so
  // fall back to a zeroed shell rather than crashing on undefined.
  const project = useMemo(() => {
    const found = buildProjects(tasks, milestones, today).find(p => p.id === projectId);
    return found || {
      id: projectId, name: projectLabel(projectId), goal: 'G1', mixedGoals: false,
      total: 0, open: 0, done: 0, inProgress: 0, blocked: 0, overdue: 0,
      msTotal: 0, msDone: 0, progress: 0,
    };
  }, [tasks, milestones, today, projectId]);

  const mine = useMemo(
    () => tasksInProject(tasks, projectId),
    [tasks, projectId],
  );
  const myMs = useMemo(
    () => milestonesInProject(milestones, tasks, projectId)
      .sort((a, b) => (a.dueDate || '9999').localeCompare(b.dueDate || '9999')),
    [milestones, tasks, projectId],
  );

  // Subtasks live under their parent here rather than as loose rows — the
  // whole point of the project view is structure, not a flat dump.
  ...
  const color = GOAL_COLORS[project.goal] || COLORS.textMuted;
      ...
      <View style={styles.sheet}>
        <View style={styles.header}>
          ...
          <Text style={[styles.goalChip, { color, borderColor: color }]}>
            {project.goal}{project.mixedGoals ? '*' : ''}
          </Text>
        </View>

        <ScrollView contentContainerStyle={styles.body}>
          <Text
            style={[styles.projectName, projectId === UNSORTED_ID && styles.nameUnsorted]}
          >
            {project.name}
          </Text>
          {project.mixedGoals && (
            <Text style={styles.mixedNote}>
              Tasks here span more than one goal — {project.goal} is just the majority.
            </Text>
          )}

          <View style={styles.track}>
            <View style={[styles.fill, {
              width: `${Math.round(project.progress * 100)}%`, backgroundColor: color,
            }]} />
          </View>

          <View style={styles.statRow}>
            <Stat label="OPEN" value={project.open} />
            <Stat label="DONE" value={project.done} color={COLORS.accent} />
            <Stat label="BLOCKED" value={project.blocked}
              color={project.blocked > 0 ? COLORS.priorityHigh : undefined} />
            <Stat label="OVERDUE" value={project.overdue}
              color={project.overdue > 0 ? COLORS.priorityHigh : undefined} />
          </View>
          ...
          {mine.length === 0 && (
            <Text style={styles.empty}>
              No tasks in this project. Set a task's SECTION to "{project.name}" to file it here.
            </Text>
          )}
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  projectName: { fontSize: 21, fontFamily: FONTS.display, color: COLORS.textPrimary },
  nameUnsorted: { color: COLORS.textMuted, fontStyle: 'italic' },
  mixedNote: {
    fontSize: 9, fontFamily: FONTS.mono, letterSpacing: 0.5,
    color: COLORS.textFaint, marginTop: 5, lineHeight: 13,
  },
  track: {
