...
// Unbound Time weekly threshold, mirroring the Obsidian MCI template's
// dataviewjs block exactly (that template is the authoritative definition;
// this is a port, not a reinterpretation): cohort = live tasks due in the
// CURRENT ISO week with priority High or Mid; threshold = 90% done AND at
// least 5 tasks in the cohort.
//
// Deliberately keyed off `dueDate` (via isoWeekTag), never off the stored
// `week` field. `week` is only written by TaskEditor.handleSave, so
// quick-added and synced tasks routinely have it blank, and a past
// Thursday-anchored tagging bug left some stored values disagreeing with
// their own dueDate. Deriving at read time is correct for every task
// regardless of how it was created; it does not require touching a single
// stored record.
//
// Subtasks are counted (no parentId filter) — Obsidian's MPL counts nested
// checkboxes as tasks, so this matches that model. If Connor later rules
// subtasks should not count toward the weekly cohort, this filter is the
// one place to change.
export const unboundTimeWeek = (tasks, todayISO) => {
  const weekTag = isoWeekTag(todayISO);
  const cohort = live(tasks).filter(t =>
    t.dueDate
    && isoWeekTag(t.dueDate) === weekTag
    && (t.priority === 'High' || t.priority === 'Mid')
  );
  const total = cohort.length;
  const done = cohort.filter(t => t.status === 'done').length;
  const pct = total > 0 ? Math.round((done / total) * 100) : 0;
  const belowMinimum = total < 5;
  const atThreshold = pct >= 90 && total >= 5;
  return { weekTag, total, done, pct, atThreshold, belowMinimum };
};

// Last `n` ISO weeks ending with the current one (oldest first), each in
// the same shape as unboundTimeWeek. `todayISO` is only used to anchor the
// window; each entry is computed by walking backward 7 days at a time and
// re-deriving from dueDate, so it is correct across a year boundary.
export const recentWeeksUnbound = (tasks, todayISO, n = 4) => {
  const out = [];
  ...
  for (let i = n - 1; i >= 0; i--) {
    ...
    out.push(unboundTimeWeek(tasks, iso));
  }
  return out;
};
