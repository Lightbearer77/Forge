...
import { normalizeTask, normalizeMilestone, normalizeRune } from './model';
...
// v3: settings key-value table (sync token, preferences).
...
const RUNES_DDL = `
  CREATE TABLE IF NOT EXISTS runes (
    ...
    domain TEXT NOT NULL DEFAULT 'practical',
    task TEXT NOT NULL DEFAULT '',
    month TEXT NOT NULL DEFAULT '',
    tag TEXT NOT NULL DEFAULT '',
    adopt_status TEXT NOT NULL DEFAULT '',
...
const MIGRATIONS = {
  ...
  3: async (db) => { await db.execAsync(SETTINGS_DDL); },
  4: async (db) => { await db.execAsync(RUNES_DDL); },
};
...
export const initDatabase = async () => {
  ...
  await db.execAsync(SETTINGS_DDL);
  await db.execAsync(RUNES_DDL);
...
// ─── Settings (key-value; sync token lives here, on-device only) ───
...
// ─── Runes ───
const rowToRune = (r) => ({
  id: r.id, name: r.name, glyph: r.glyph, domain: r.domain,
  task: r.task, month: r.month, tag: r.tag, adoptStatus: r.adopt_status,
  taskId: r.task_id, earned: !!r.earned, earnedAt: r.earned_at,
  sortOrder: r.sort_order || 0, deleted: !!r.deleted,
  createdAt: r.created_at, updatedAt: r.updated_at,
});

const RUNE_UPSERT_SQL = `
  INSERT INTO runes (id, name, glyph, domain, task, month, tag,
    adopt_status, task_id, earned, earned_at, sort_order, deleted,
    created_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ON CONFLICT(id) DO UPDATE SET
    name = excluded.name, glyph = excluded.glyph, domain = excluded.domain,
    task = excluded.task, month = excluded.month, tag = excluded.tag,
    adopt_status = excluded.adopt_status, task_id = excluded.task_id,
    earned = excluded.earned, earned_at = excluded.earned_at,
    sort_order = excluded.sort_order, deleted = excluded.deleted,
    updated_at = excluded.updated_at
`;

const runeToParams = (r) => [
  r.id, r.name, r.glyph, r.domain, r.task, r.month, r.tag,
  r.adoptStatus || '', r.taskId || '', r.earned ? 1 : 0, r.earnedAt || '',
  r.sortOrder || 0, r.deleted ? 1 : 0, r.createdAt || Date.now(), r.updatedAt || Date.now(),
];

export const getAllRunes = async ({ includeDeleted = false } = {}) => {
  const db = await getDb();
  const rows = await db.getAllAsync(
    includeDeleted
      ? 'SELECT * FROM runes ORDER BY sort_order, created_at'
      : 'SELECT * FROM runes WHERE deleted = 0 ORDER BY sort_order, created_at'
  );
  return rows.map(rowToRune);
};

export const saveRune = async (rune) => {
  const r = normalizeRune(rune);
  if (!r) return;
  const db = await getDb();
  await db.runAsync(RUNE_UPSERT_SQL, runeToParams(r));
};

export const saveRunes = async (list) => {
  const db = await getDb();
  await db.withTransactionAsync(async () => {
...
const RUNE_SEED_TS = 1000000000000;
export const ensureRunesSeeded = async () => {
  if ((await getSetting('runesSeeded')) === '1') return;
  const { NU_2026_RUNE_ASSIGNMENTS, RUNE_GLYPHS } = await import('./runeData');
  const seeded = NU_2026_RUNE_ASSIGNMENTS.map((r, i) => ({
    id: `rune_${r.name}`,
    name: r.name,
    glyph: RUNE_GLYPHS[r.name] || '',
    domain: r.domain,
    task: r.task,
    month: r.month,
    tag: r.tag,
    adoptStatus: r.adoptStatus,
    taskId: '',
    earned: false,
    earnedAt: '',
    sortOrder: i,
    createdAt: RUNE_SEED_TS,
    updatedAt: RUNE_SEED_TS,
  }));
  await saveRunes(seeded);
  await setSetting('runesSeeded', '1');
};
