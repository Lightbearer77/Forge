...
export const mergeSyncFile = (localTasks, fileJson, localMilestones = [], localRunes = []) => {
  ...
  const r = mergeCollection(localRunes, fileJson?.runes, normalizeRune);
  const report = { ...t.report, milestones: m.report, runes: r.report };
  return {
    merged: t.merged,
    changed: t.changed,
    milestones: { merged: m.merged, changed: m.changed },
    runes: { merged: r.merged, changed: r.changed },
    report,
  };
};

// Serialize for the outbound direction (and for tests / snapshots).
export const serializeTasks = (tasks, updatedBy = 'Forge', milestones = undefined, runes = undefined) => ({
...
export const pushSyncFile = async ({ token, tasks, milestones, runes = [] }) => {
  ...
  const body = serializeTasks(tasks, 'Forge', milestones, runes);
  const payload = {
    message: `Forge push: ${tasks.length} tasks, ${milestones.length} milestones, ${runes.length} runes`,
  ...
  return { pushed: tasks.length + milestones.length + runes.length };
};
